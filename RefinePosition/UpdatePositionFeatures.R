...
classPosition <- c( 'NO_LABEL', 'Non-Position', 'Other',  'TA', 'Teacher')
...
features[,1] <- train[,1]
features <- features[,c(1,7,12:279)]
colnum <- 271
for (var in ExplanVar){
...
library(caret)
  features <- read.csv('trainFeatures.csv')
...
dat = read.csv('/home/michael/Data/EdClassification/PolishedPresentation/Submission/submit5.csv')
